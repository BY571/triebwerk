...
void InferenceEngine::batch_gemm_q4l(half* out, const NF4Weight& w, const half* in,
    ...
    if (w.fp16_cache) {
        ...
        // NF4: per-sequence dp4a GEMVs (batch GEMV has correctness issue for SSM arch)
        for (int g = 0; g < N; g++) {
...
std::vector<std::vector<int>> InferenceEngine::generate_batch(
    ...
    // CUDA graph: capture decode step for replay. Disabled for SSM until graph correctness verified.
